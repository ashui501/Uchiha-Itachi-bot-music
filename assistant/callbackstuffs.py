...
try:
    from CythonX.fns.gDrive import GDriveManager
except ImportError:
    GDriveManager = None
...
from CythonX.fns.helper import fast_download, progress
from CythonX.fns.tools import Carbon, async_searcher, get_paste, telegraph_client
from CythonX.startup.loader import Loader
...
_buttons = {
    "otvars": {
        "text": "Other Variables to set for CɪᴘʜᴇʀX ᴇxᴄlusivᴇ ʙᴏᴛ:",
        "buttons": [
            [
...
async def update(eve):
    ...
    if heroku_api:
        ...
        execl(sys.executable, sys.executable, "-m", "CythonX")
...
async def _(e):
    ...
    async with asst.conversation(e.sender_id) as conv:
        ...
        if GDrive._create_token_file(code=code.text):
            await conv.send_message(
                "`Success!\nYou are all set to use Google Drive with CɪᴘʜᴇʀX Ⲉⲭⲥⳑυⲋⲓⳳⲉ Ⲃⲟⲧ.`",
