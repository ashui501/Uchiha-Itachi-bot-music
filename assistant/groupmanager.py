...
async def ban(event):
    ...
    userids = []
    async for user in asst.iter_participants(
...
async def nothanos(event):
    ...
    creator = chat.creator
    if not admin and not creator:
        await event.reply("I'm Not an Admin 🥺")
        return
...
async def pin(event):
    ...
    options = event.pattern_match.group(0).decode("UTF-8")
    is_silent = True
...
async def get_user_from_event(event):
    ...
    args = event.pattern_match.group(0).split(" ", 1)
    extra = None
    if event.reply_to_msg_id:
        previous_message = await event.get_reply_message()
        user_obj = await event.client.get_entity(previous_message.from_id)
        extra = event.pattern_match.group(0).decode("UTF-8")
    elif args:
        user = args[0]
        if len(args) == 2:
            extra = args[1]
