...
from cython.functions.logusers_db import *
...
async def _(e):
    ...
    if is_logger(str(e.chat_id)):
        ...
        return await eod(e, "`Now I Will log mssages from here.`", time=1)
    else:
        return await eod(e, "`Wasn't logging mssages from here.`", time=1)
...
async def _(e):
    ...
    if not is_logger(str(e.chat_id)):
        ...
        return await eod(e, "`Now I Won't log mssages from here.`", time=1)
    else:
        return await eod(e, "`Wasn't logging mssages from here.`", time=1)
